...
/* ── Ambil role dari profiles / metadata ────────────────────── */
...
async function doLogin() {
  ...
  if (!email || !password) { showError('Harap isi email dan kata sandi.'); return; }
  ...
  try {
    ...
    if (signInError) {
      await insertLoginHistory(email, 'failed'); // dari supabase.js
      setLoading(false);
    ...
    if (!userId) {
      await insertLoginHistory(email, 'failed');
      setLoading(false);
    ...
    if (role !== 'admin') {
      ...
      await insertLoginHistory(email, 'failed');
      setLoading(false);
      showError(role ? 'Akun ini tidak memiliki akses admin.' : 'Profil admin tidak ditemukan.');
      return;
    }

    // 3. Sukses
    await insertLoginHistory(email, 'success');
    setLoading(false);
    showToast('Berhasil masuk. Mengalihkan...', 'success');
    setTimeout(() => { window.location.href = 'index-admin.html'; }, 800);
...
async function checkExistingSession() {
  try {
    ...
    if (role === 'admin') window.location.href = 'index-admin.html';
  } catch (err) {
