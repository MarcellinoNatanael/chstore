...
// =============================================
// LOGIN HISTORY — helper global
// Dipanggil dari login-admin.js
// Insert pakai REST fetch langsung agar tidak
// terpengaruh state session Supabase client
// =============================================
async function insertLoginHistory(email, status) {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/login_history`, {
      method:  'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      console.warn('insertLoginHistory gagal:', res.status, txt);
    } else {
      console.log('login_history tercatat:', status, email);
    }
  } catch (e) {
    console.warn('insertLoginHistory exception:', e);
  }
}
