...
function starsHtml(rating, size = 16) {
  let h = '';
  for (let i = 1; i <= 5; i++) {
    h += `<span class="pd-star ${i <= rating ? 'filled' : ''}" style="font-size:${size}px">★</span>`;
  }
  return h;
}

function anonName(idx) {
  const names = ['Pembeli Terverifikasi', 'Pelanggan Setia', 'Pengguna CHStore', 'Pembeli CHStore', 'Member CHStore'];
  return names[idx % names.length];
}

function anonAvatar(idx) {
...
function closeLightbox() {
  document.getElementById('pdLightbox').style.display = 'none';
...
function buildGallery(images) {
  ...
  if (!images.length) {
    mainEl.src = 'https://placehold.co/480x480/f5f5f5/1d1d1f?text=No+Image';
    return;
  }

  const primary = images.find(i => i.is_primary) || images[0];
  mainEl.src = primary.image_url;
  mainEl.alt = product?.name || '';

  thumbsEl.innerHTML = images.map((img, i) => `
    <div class="pd-thumb ${i === 0 ? 'active' : ''}"
      onclick="setMainImage('${escHtml(img.image_url)}', ${i})" title="Foto ${i + 1}">
      <img src="${escHtml(img.image_url)}" alt="Foto ${i + 1}" loading="lazy">
    </div>`).join('');

  document.getElementById('pdMainImg').onclick = () =>
    openLightbox(allImages[currentImgIdx]?.image_url || allImages[0]?.image_url);
}

/* ── Condition Badge ────────────────────────────────────────── */
function setCondition(cond) {
  const map = {
    baru:        { label: 'Baru',          cls: 'badge-baru',        tag: '✦ Baru' },
    second:      { label: 'Second',        cls: 'badge-second',      tag: '✦ Second' },
    baru_second: { label: 'Baru / Second', cls: 'badge-baru-second', tag: '✦ Baru / Second' },
  };
  const c = map[cond] || map['baru'];
  const badge = document.getElementById('pdConditionBadge');
  const tag   = document.getElementById('pdConditionTag');
  badge.className   = 'pd-condition-badge ' + c.cls;
  ...
  if (colors.length > 0) {
    ...
    document.getElementById('pdColors').innerHTML = colors.map(v => `
      <button class="pd-color-btn"
        data-hex="${escHtml(v.color_hex)}"
        data-name="${escHtml(v.color_name || '')}"
        style="background:${escHtml(v.color_hex)}"
        title="${escHtml(v.color_name || v.color_hex)}"
        onclick="selectColor('${escHtml(v.color_hex)}','${escHtml(v.color_name || '')}')">
      </button>`).join('');
    const first = colors[0];
    selectColor(first.color_hex, first.color_name || '');
  }
}

function selectColor(hex, name) {
  selectedColor = hex;
  document.getElementById('selectedColorName').textContent = name || hex;
...
function buildSizes(sizes) {
  ...
  document.getElementById('pdSizes').innerHTML = sizes.map((s, i) => `
    <button class="pd-size-btn ${i === 0 ? 'active' : ''}"
      data-size="${escHtml(s)}"
      onclick="selectSize('${escHtml(s)}', this)">
      ${escHtml(s)}
    </button>`).join('');
...
function updatePrice() {
  ...
  if (selectedColor) {
    const v = variants.find(v => v.color_hex === selectedColor);
    if (v?.price_diff) price += v.price_diff;
  }
...
function renderRatingSummary(reviews) {
  const starsRow   = document.getElementById('pdStarsRow');
  const ratingCount = document.getElementById('pdRatingCount');
  ...
  if (!reviews?.length) {
    ...
    avgStars.innerHTML    = '';
    avgCount.textContent  = 'Belum ada ulasan';
    return;
  }
  const avg     = reviews.reduce((s, r) => s + (r.rating || 0), 0) / reviews.length;
  const rounded = Math.round(avg * 10) / 10;
  starsRow.innerHTML      = starsHtml(Math.round(avg), 15);
  ratingCount.textContent = `${rounded} (${reviews.length} ulasan)`;
  avgScore.textContent    = rounded.toFixed(1);
  avgStars.innerHTML      = starsHtml(Math.round(avg), 18);
  avgCount.textContent    = `${reviews.length} ulasan`;
}
...
function initWA() {
  ...
  // Tombol WA — Tanya Stok
  const btnWA = document.getElementById('pdBtnWA');
  if (btnWA) {
    btnWA.onclick = () => {
      const color = selectedColor
        ? ' Warna: ' + (document.getElementById('selectedColorName')?.textContent || '')
        : '';
  ...
  if (btnCart) {
    btnCart.onclick = () => {
      const colorName = selectedColor
        ...
        : '';
      const img = allImages.find(i => i.is_primary)?.image_url
      ...
      showToast('Ditambahkan ke cart 🛍️', 'success');
    };
  }
}
  ...
  list.innerHTML = reviews.map((r, i) => {
    ...
    return `
      <div class="pd-review-card">
        ...
        ${imgs.length ? `<div class="pd-review-imgs">${imgs.map(url =>
          `<img src="${escHtml(url)}" class="pd-review-img" alt="Foto review"
            loading="lazy" onclick="openLightbox('${escHtml(url)}')">`
        ).join('')}</div>` : ''}
      </div>`;
  }).join('');
}
...
function initReviewForm(productId) {
  ...
  const starLabel = document.getElementById('starValLabel');
  ...
  imgInput.addEventListener('change', () => {
    Array.from(imgInput.files).forEach(file => {
      ...
      r.onload = e => { reviewImages.push({ file, preview: e.target.result }); renderReviewImgPreview(); };
...
async function submitReview(productId) {
  ...
  try {
    ...
    for (const img of reviewImages) {
      const url = await uploadReviewImg(img.file);
    ...
    document.getElementById('starValLabel').textContent = 'Pilih bintang';
...
async function uploadReviewImg(file) {
  ...
  const { error } = await supabaseClient.storage
    .from('products').upload(filename, file, { cacheControl: '3600', upsert: false });
...
async function loadProduct() {
  ...
  try {
    ...
    document.getElementById('pageDesc').content =
      data.description || `${data.name} tersedia di CHStore Apple Premium Reseller Jakarta`;

    // Breadcrumb
    const cat = data.categories;
    if (cat) {
      const breadCat = document.getElementById('breadCat');
      breadCat.textContent = cat.name;
      breadCat.href = `product.html?category=${cat.slug}`;
    }
    ...
    if (data.description) {
      ...
      document.getElementById('pdDesc').textContent = data.description;
    }
    ...
    if (specs.length) {
      ...
      document.getElementById('pdSpecsTable').innerHTML = specs.map(s =>
        `<tr><td>${escHtml(s.spec_key)}</td><td>${escHtml(s.spec_value)}</td></tr>`
      ).join('');
    }
